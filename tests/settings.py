...
class TestSettingsWithPythonValues(unittest.TestCase):
    ...
    def test_well_formed_values(self):
        settings = Settings(cdn_host=HttpUrl("http://cdn-domain"), redirect_ratio=parse_redirect_ratio("1:10"))
        self.assertEqual(settings.cdn_host.host, "cdn-domain")
        self.assertEqual(settings.redirect_ratio, Fraction(1, 10))

    def test_ill_formed_value_for_cdn_host(self):
        with self.assertRaises(ValidationError):
            Settings(cdn_host=HttpUrl("123456789"), redirect_ratio=parse_redirect_ratio("1:10"))

    def test_non_http_cdn_host(self):
        with self.assertRaises(ValidationError):
            Settings(cdn_host=HttpUrl("ftp://cdn-domain"), redirect_ratio=parse_redirect_ratio("1:10"))
    ...
    def test_negative_cdn_redirect_count(self):
        with self.assertRaises(ValidationError):
            Settings(cdn_host=HttpUrl("http://cdn-domain"), redirect_ratio=parse_redirect_ratio("-1:10"))
...
class TestSettingsFromEnv(unittest.TestCase):
    ...
    @patch_environ(BALANCER_CDN_HOST="http://cdn-domain", BALANCER_REDIRECT_RATIO="1:10")
    def test_well_formed_values(self):
        settings = Settings()  # type: ignore
        self.assertEqual(settings.cdn_host.host, "cdn-domain")
        self.assertEqual(settings.redirect_ratio, Fraction(1, 10))

    @patch_environ(BALANCER_CDN_HOST="123456789", BALANCER_REDIRECT_RATIO="1:10")
    def test_ill_formed_value_for_cdn_host(self):
        with self.assertRaises(ValueError):
            Settings()  # type: ignore

    @patch_environ(BALANCER_CDN_HOST="ftp://cdn-domain", BALANCER_REDIRECT_RATIO="1:10")
    def test_non_http_cdn_host(self):
        with self.assertRaises(ValueError):
            Settings()  # type: ignore
